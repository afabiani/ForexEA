...
//  double COMmultiplier=10;
   ...
   if((UseTrailStop) && (UseAwesomeTrail))
     {
         ...
         if(OrderType()==OP_SELL) 
           {
            ...
            if(TrailingStop>0) 
              {
               if(((NormalizeDouble((Ask+(TSActivationPips*Point*DigiMultiplier)),dgts)<NormalizeDouble(OrderOpenPrice(),dgts))
               && (NormalizeDouble(OrderStopLoss(),dgts)>NormalizeDouble(Ask+((TrailingStep+TrailingStop)*Point*DigiMultiplier),dgts))) || (DelayTS && (OrderStopLoss()==0))) 
                 {
                  if(!OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(Ask+((TrailingStep+TrailingStop)*Point*DigiMultiplier),dgts),0,0,Red))
                    {
         ...
         if(OrderType()==OP_BUY) 
           {
            ...
            if(TrailingStop>0) 
              {
               if(((NormalizeDouble((Bid-(TSActivationPips*Point*DigiMultiplier)),dgts)>NormalizeDouble(OrderOpenPrice(),dgts))
                 
                 && (NormalizeDouble(OrderStopLoss(),dgts)<NormalizeDouble(Bid-((TrailingStep+TrailingStop)*Point*DigiMultiplier),dgts))) || (DelayTS && (OrderStopLoss()==0))) 
                  {
                  if(!OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(Bid-((TrailingStep+TrailingStop)*Point*DigiMultiplier),dgts),0,0,Green))
                    {
                     Print("OrderModify failed");
                    }
                 }
            ...
            if(OrderType()==OP_SELL) 
              {
               ...
               Open_Sells++;
                  ...
                  if( UseAwesomeTrail &&(Ask < NormalizeDouble(LowestUpFrac+(TrailingStop*Point*DigiMultiplier),dgts)) 
                    && Ask < NormalizeDouble((OrderStopLoss()- (TrailingStop*Point*DigiMultiplier)),Digits)
                    && UpFracTime > OrderOpenTime()+Shift*60
                    && NormalizeDouble(LowestUpFrac+(TrailingStop*Point*DigiMultiplier),dgts) < OrderStopLoss())
                    {
                     Print("Lowest Up Frac  ",LowestUpFrac);
                      if(!OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(LowestUpFrac+(TrailingStop*Point*DigiMultiplier),dgts),0,0,Red))
                       {
                        Print("OrderModify failed");
                       }
                    }
            ...
            if(OrderType()==OP_BUY) 
              {
               ...
               Open_Buys++;
                  ...
                  if(UseAwesomeTrail &&(Bid > NormalizeDouble(HighestDownFrac-(TrailingStop*Point*DigiMultiplier),dgts) ) 
                  && Bid > NormalizeDouble((OrderStopLoss()+ (TrailingStop*Point*DigiMultiplier)),Digits)
                  && DownFracTime > OrderOpenTime()+Shift*60
                  && NormalizeDouble(HighestDownFrac-(TrailingStop*Point*DigiMultiplier),dgts) > OrderStopLoss())
                    {
                    Print("Highest Down Frac  ",HighestDownFrac);
                      if(!OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(HighestDownFrac-(TrailingStop*Point*DigiMultiplier),dgts),0,0,Red))
                       {
                        Print("OrderModify failed");
                       }
                    }
