...
int DELETEP(string symbol_c, string penType)
{
   ...
   if(penType=="All")
   {
      for(cnt=0;cnt<retrynum;cnt++)
      {
         if(CountPendingOrders(symbol_c,OP_BUYSTOP,MagicNumber)>0)
         {
            DeletePending(symbol_c,OP_BUYSTOP,MagicNumber);
         }
         
         if(CountPendingOrders(symbol_c,OP_SELLSTOP,MagicNumber)>0)
         {
            DeletePending(symbol_c,OP_SELLSTOP,MagicNumber);
         }

         if(CountPendingOrders(symbol_c,OP_BUYLIMIT,MagicNumber)>0)
         {
            DeletePending(symbol_c,OP_BUYLIMIT,MagicNumber);
         }

         if(CountPendingOrders(symbol_c,OP_SELLLIMIT,MagicNumber)>0)
         {
            DeletePending(symbol_c,OP_SELLLIMIT,MagicNumber);
         }
      }
...
int CountPendingOrders(string symbolclose,int ordertypeclose,int magicclose)
{
   int num_orders = 0;
   int cnt;
   int orderstotal=OrdersTotal();
