...
//-- Check if trading is possible
   ...
   if(UseTradingTime)
   {
      ...
      {
         if(IsNewBar) Print(" INFO: Trading Time oustide the range. Do not Trade anymore!");
   ...
   if (AccelerationPips > 0)
   {
      ...
      {
         Print(" WARNING: Market too fast!! SKIP Trading due to HIGH Acceleration -> Acceleration Pips: ", MathAbs(NormalizeDouble(Acceleration[0]/(DigiMultiplier*Point),2)));
         return;
      }
   }
   //----------------------------------------------
    ...
    for (int cnt = 0; cnt < OrdersTotal(); cnt++)
    {
        ...
        if ((OrderMagicNumber() == MagicNumber) && (OrderSymbol() == Symbol()))
        {
            ...
            if (OrderType() == OP_SELL)
            {
                ...
                if (UseTrailStop && TrailingStop > 0)
                {
                    if (ProfitLock)
                    {
                        if (AccountEquity() >= (AccountBalance() + (AccountBalance() * ProLockPer / 100.0)))
...
void CheckAwesomeStop() {
   ...
   if(UseAwesomeTrail || UseBE)
   {
       RefreshRates();
   
       if (ProfitLock && (AccountEquity() >= (AccountBalance() + (AccountBalance() * ProLockPer / 100.0))))
       {
            return;
       }
       ...
       for (int cnt = 0; cnt < OrdersTotal(); cnt++)
       {
           ...
           if ((OrderMagicNumber() == MagicNumber) && (OrderSymbol() == Symbol()))
           {
               ...
               if(UseAwesomeATRLevels)
               {
                  ...
                  double fullatrx = iATR(Symbol(), 0, ATRLevelsATRPeriod, 1);
               ...
               if (OrderType() == OP_SELL)
               {
                   ...
                   if (UseAwesomeTrail)
                   {
                      if(!UseAwesomeATRLevels && !UseAwesomeSMMALevels)
                      {
                      ...
                      if(UseAwesomeSMMALevels) {
                          
                          double bb1_ma = iMA(NULL,0,SMMALevelsMAPeriod,0,MODE_SMMA,PRICE_MEDIAN,1);
               ...
               if (OrderType() == OP_BUY)
               {
                   ...
                   if (UseAwesomeTrail)
                   {
                      ...
                      if(UseAwesomeSMMALevels) {
                          bb1_ma = iMA(NULL,0,SMMALevelsMAPeriod,0,MODE_SMMA,PRICE_MEDIAN,1);
